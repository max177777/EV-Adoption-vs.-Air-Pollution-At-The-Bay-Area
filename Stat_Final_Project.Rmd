...
**Load the local air data, so we don't need to run the API everytime**
```{r}
suppressWarnings(air_quality_data <- read_csv("C:/Users/max_x/ucb/MaCSS - Final Project/air_quality_data_2011_2023.csv"))
```
...
```{r}
# Filter data for 2011–2024
zev_sales_filtered <- zev_sales_filtered %>%
  filter(Data_Year >= 2010 & Data_Year <= 2024)
...
ggplot(zev_sales_filtered, aes(x = Data_Year, y = Sale_for_Zero_Emission_Vehicles, color = County)) +
  geom_line(size = 1) +
  geom_point() +
  geom_vline(xintercept = 2023, linetype = "dashed", color = "red") + # Dashed line for 2024 cutoff
  labs(
    title = "Trends in Zero-Emission Vehicle Sales by County (2024 only includes first half year)",
    x = "Year",
    y = "Number of Zero-Emission Vehicles Sold",
    color = "County"
  ) +
...
**Set up the population density and the industrial activity level based on urban level** 
```{r}
# Population density data (people per square mile)
population_density <- data.frame(
  County = c("Alameda", "Contra Costa", "Marin", "Napa", 
             "San Mateo", "Santa Clara", "Solano", "Sonoma", "San Francisco"),
  Population_Density = c(2172, 1533, 498, 188, 1683, 1460, 523, 316, 18132))

# Assign industrial activity levels
# Assign industrial activity levels
population_density <- population_density %>%
  mutate(Industrial_Activity = case_when(
    County == "San Francisco" ~ "Urban",
    County %in% c("Alameda", "San Mateo", "Contra Costa", "Santa Clara") ~ "Suburban",
    County %in% c("Solano", "Marin", "Sonoma", "Napa") ~ "Rural"
  ))

# Merge population density with EV adoption and air quality data
combined_data <- ev_adoption_rate %>%
  left_join(yearly_mean_aqi, by = c("County", "Data Year" = "Year")) %>%
  left_join(population_density, by = "County")
# Inspect combined data
summary(combined_data)
```

**Linear Regression to prove DAG**
```{r}
# Fit the linear regression model
model <- lm(Mean_AQI ~ Adoption_Rate + Industrial_Activity, data = combined_data)
summary(model)
```

**Random Effects Model for DAG**
```{r}
# Random effects model
model_re <- lmer(Mean_AQI ~ Adoption_Rate + Industrial_Activity + (1 | County), data = combined_data)
summary(model_re)
```

**Analysis of Variance (ANOVA)**
```{r}
shapiro.test(combined_data$Mean_AQI[combined_data$Industrial_Activity == "Urban"])
shapiro.test(combined_data$Mean_AQI[combined_data$Industrial_Activity == "Suburban"])
shapiro.test(combined_data$Mean_AQI[combined_data$Industrial_Activity == "Rural"])

leveneTest(Mean_AQI ~ Industrial_Activity, data = combined_data)

anova_result <- aov(Mean_AQI ~ Industrial_Activity, data = combined_data)
...
#urban_aqi <- air_quality_data_cleaned %>% filter(County == "San Francisco")
...
# Perform t-test (Urban vs Suburban)
#t_test_result_urban_suburban <- t.test(urban_aqi$AQI, suburban_aqi$AQI)
#print(t_test_result_urban_suburban)

# Perform t-test (Urban vs Rural)
#t_test_result_urban_rural <- t.test(urban_aqi$AQI, rural_aqi$AQI)
#print(t_test_result_urban_rural)

# Perform t-test (Suburban vs Rural)
#t_test_result_suburban_rural <- t.test(suburban_aqi$AQI, rural_aqi$AQI)
#print(t_test_result_suburban_rural)
```
...
#    
#    "Test Statistic:\n",
#    "t = -24.295: The t-value indicates the difference between the group means relative to the variability in the data.\n",
#    "A large absolute t-value (like -24.295) suggests a significant difference between the two groups.\n\n",
#    
#   "Degrees of Freedom:\n",
#  "df = 28727: The degrees of freedom approximate the sample size and variability in the data.\n\n",
#    
#    "P-value:\n",
...
#    
#    "Confidence Interval:\n",
#    "95% Confidence Interval: [-2.985688, -2.539897]:\n",
#    "The difference in mean AQI between urban and suburban areas is estimated to fall between -2.99 and -2.54 with 95% confidence.\n",
#    "Since the confidence interval does not include 0, it further supports rejecting the null hypothesis.\n\n",
#    
#    "Sample Means:\n",
#    "Mean of x (urban): 23.50.\n",
#    "Mean of y (suburban): 26.26.\n",
...
**More Visualization**
```{r}
ggplot(yearly_mean_aqi, aes(x = Year, y = Mean_AQI, color = County)) +
  geom_line(size = 1) +
  geom_point() +
  labs(title = "Yearly Trends in Mean AQI by County",
    x = "Year",
    y = "Mean AQI",
    color = "County") +
  theme_minimal()
...
```{r}
ggplot(air_quality_data_cleaned, aes(x = Date, y = AQI, color = Pollutant)) +
  geom_line(alpha = 0.7) +
  facet_wrap(~County, scales = "free_y") +
  labs(
    title = "Daily AQI Trends by Pollutant and County",
    x = "Date",
    y = "AQI",
    color = "Pollutant"
  ) +
...
```{r}
ggplot(air_quality_data_cleaned, aes(x = County, y = AQI, fill = County)) +
  geom_boxplot() +
  labs(
    title = "AQI Distribution by County",
    x = "County",
    y = "AQI"
  ) +
  theme_minimal() +
...
**Visualizations for Multiple Regression Model**
```{r}
merged_data <- merged_data %>%
  mutate(Predicted_AQI = predict(regression_model))

ggplot(merged_data, aes(x = Predicted_AQI, y = Mean_AQI, color = County)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "blue") +
  labs(
    title = "Predicted vs Observed Mean AQI",
    x = "Predicted Mean AQI",
    y = "Observed Mean AQI"
  ) +
  theme_minimal()
```

**Effect of EV Adoption Rate by Region**
```{r}
ggplot(merged_data, aes(x = Adoption_Rate, y = Mean_AQI, color = Region)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  facet_wrap(~Region, scales = "free_y") +
  labs(
    title = "Effect of EV Adoption Rate on AQI by Region",
    x = "EV Adoption Rate (%)",
    y = "Mean AQI"
  ) +
  theme_minimal()
 ...
 **Residual Analysis**
```{r}
ggplot(merged_data, aes(x = Predicted_AQI, y = residuals(regression_model))) +
  geom_point() +
